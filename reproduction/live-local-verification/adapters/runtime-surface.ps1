...
try {
  ...
  Write-JsonFile (Join-Path $BundleRoot "versions.json") ([ordered]@{ flowplane = Get-GitState $FlowplaneRoot; integration = $integrationId; runtimeImage = $runtimeImage; runtimeImageId = $runtimeInspect.Image; bridgeImageId = $bridgeInspect.Image; runtimeSurfaceBridgeSha256 = if ($isEmbedded) { $null } else { Get-Sha256 $bridgeScript }; rawOnlyVerifierSha256 = Get-Sha256 $verifierScript; runtimeJarSha256 = if ($isSidecar) { Get-Sha256 $sidecarJar } elseif ($isEmbedded) { Get-Sha256 $embeddedJar } else { $null }; nodeVersion = (& node --version); dockerVersion = (& docker version --format '{{.Server.Version}}') })
