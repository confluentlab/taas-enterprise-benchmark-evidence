...
A runtime can have more than one status because the boundaries differ. For example, the gRPC fixture is `CONTRACT_VERIFIED`, while the separate live service attempt is `PRESERVED_FAILURE`. Neither status cancels the other.
